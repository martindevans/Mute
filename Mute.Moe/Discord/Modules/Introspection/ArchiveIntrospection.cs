...
[RequireOwner]
[Group("archive")]
public class ArchiveIntrospection(IChatArchiveHoles _holes, IChatArchive _archive)
    : MuteBaseModule
{
    ...
    public async Task ChannelHoles()
    {
        if (Context.Guild is { } guild)
        {
            ...
            foreach (var channel in guild.Channels)
            {
                ...
                {
                    builder.Append($"**#[{channel.Name}]({DiscordLinks.Channel(channel)})**: {channelHoles.Count} hole{(channelHoles.Count == 1 ? "" : "s")}\n");
    ...
    private async Task<string> FormatMessage(ChatArchiveHole hole, IMessageChannel channel)
    {
        ...
        var linktext = $"`{date}`";
        var link = message == null ? $"{linktext}" : $"[{linktext}]({DiscordLinks.Message(message)})";

        return $"{link} ({(hole.Forward ? "forward" : "backward")})";
    }
}
