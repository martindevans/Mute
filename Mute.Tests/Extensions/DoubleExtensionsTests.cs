...
public class DoubleExtensionsTests
{
    ...
    public void LogitToProbability_Double_Fuzz()
    {
        ...
        for (var i = 0; i < iterations; i++)
        {
            ...
            Assert.IsTrue(p is >= 0.0 and <= 1.0, $"Probability {p} for logit {logit} is out of [0,1]");
    ...
    public void LogitToProbability_Float_Fuzz()
    {
        ...
        for (var i = 0; i < iterations; i++)
        {
            ...
            Assert.IsTrue(p is >= 0.0f and <= 1.0f, $"Probability {p} for logit {logit} is out of [0,1]");
            // sigmoid(-x) == 1 - sigmoid(x)
